...
function antStep(who, boxArr, antArr, mePlayer, taboo, dry){
  ...
  if(me.claim>=0 && (boxArr.indexOf(me.claim)<0 || me.claim===taboo)){ me.claim=-1; me.dir=-1; }
...
async function playAnts(seq, done){
  ...
  for(let who=0; who<coants.length; who++){
    ...
    for(let i=1;i<path.length;i++){
      ...
      paint();
      if(cellEls[plan.box]) cellEls[plan.box].classList.add('antbox');
    ...
    paint();
    if(cellEls[plan.to]) cellEls[plan.to].classList.add('antbox');
...
const RULE_TEXT={
  ...
  ants:'ステージには<b>同僚のアリ</b>がいます（色が薄いほう）。濃いほうがあなたです。'
       +'あなたが '+SKIN.box+' を1つ押すと、<b>同僚もひとりずつ動きます</b>。'
       +'同僚は<b>自分から歩いて一番近い '+SKIN.box+'</b> のところまで行き、'
       +'<b>歩いてきた向きのまま</b>1マス押します。回り込んで横から押すことはありません。'
       +'押せないときはその場にいます。同僚のマスには入れず、そこへ押し込むこともできません。',
