...
const GLIDE_MS=720;                       // 冬の氷。1マスぶん滑るときの時間
/* 何マスも滑るときに、同じ時間で行かせると速すぎる。
   かといって距離ぶんだけ時間を伸ばすと、遠いときに間延びする。
   間をとって、距離の0.85乗で伸ばす。1マスは0.72秒、4マスで約2.2秒。
   長すぎても待たされるので上限を置く */
const glideMs=n=>Math.min(2800, Math.round(GLIDE_MS*Math.pow(Math.max(1,n||1), 0.85)));
