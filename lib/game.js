...
function parseBoard(board){
  ...
  const free=[], open=[], num=[], slot=[], players=[];   // free/open は印のない荷物と置き場
  let player=-1;
  for(let y=0;y<h;y++){
  ...
  return num.length
    ? {grid, water, w, h,
       ...
       players, player}
    : {grid, water, w, h, boxes:free.sort((a,b)=>a-b), goals:open.sort((a,b)=>a-b), players, player};
}
...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    fig.textContent = box ? SKIN.box : (i===player||waiting ? SKIN.man : '');
    if(RULE==='duo'){
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    undoStack.push({player, mate, boxes:boxes.slice(), pushCount, moveCount, filled:new Set(filled), rot:rot.slice()});
    ...
    undoStack.push({player, mate, boxes:boxes.slice(), pushCount, moveCount});
    player=next;
    moveCount++;
