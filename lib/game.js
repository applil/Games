...
document.body.insertAdjacentHTML("afterbegin", `<div class="app">
  ...
  <div class="debugbar">
    ...
    <button class="btn" id="btnEditEnter">面を編集</button>
    ...
    <button class="btn" id="btnEditExit">編集をやめる</button>
    ...
    <button class="btn" id="btnDiag">止まった記録</button>
    <button class="btn" id="btnSoundOff">音を止める</button>
    <button class="btn" id="btnAnimOff">動きを止める</button>
  ...
  <div class="board-wrap">
    ...
    <!-- お知らせは盤の上に重ねる。ここに置くと場所を取らないので、
         出入りしても盤の大きさが変わらない(前は19%も伸び縮みしていた) -->
  ...
  <div class="palette" id="palette">
    ...
    <button data-tool="box1" class="pal-mark">荷物A</button>
    <button data-tool="box2" class="pal-mark">荷物B</button>
    <button data-tool="slot1" class="pal-mark">置き場A</button>
    <button data-tool="slot2" class="pal-mark">置き場B</button>
    <button data-tool="box3" class="pal-mark">荷物C</button>
    <button data-tool="slot3" class="pal-mark">置き場C</button>
    <button data-tool="water" class="pal-water">水</button>
    <button data-tool="ant" class="pal-ant">同僚のアリ</button>
    <button data-tool="mate" class="pal-duo">ミツバチを足す</button>
    <button data-tool="man">🐥</button>
  </div>

  <div class="boardnote" id="boardNote"></div>
  <div class="boardnote" id="diagNote" style="display:none;white-space:pre-wrap;text-align:left"></div>
...
<div class="overlay" id="rulesOverlay">
  <div class="win-card" style="text-align:left">
    ...
    <div class="rules">
      ...
      <p>置き場は<span class="sw goal"></span><b>破線の輪</b>のマスです。荷物が乗ると<span class="sw done"></span><b>実線の輪</b>に変わります。<b>全部が実線の輪になればクリア</b>です。（色でも分かれています）</p>
...
<div class="overlay" id="modeOverlay">
  <div class="picker">
    ...
    <button class="btn" id="btnModeClose">とじる</button>
  </div>
</div>
...
const PACKS={
  summer:  {file:'packs/summer.json',   skin:'summer',   label:'夏・水',  rule:'water',
            ...
            box:'📦',
            dir:'water', emoji:'🦀📦', name:'カニと水',
            desc:'ダンボールは水に入りません。カニは歩けます'},
  squirrel:{file:'packs/squirrel.json', skin:'squirrel', label:'リス・穴', rule:'holes',
            dir:'hole', emoji:'🐿️🌰', name:'リスと穴',
            desc:'置き場は穴。ドングリを埋めるまで通れません'},
  ice:     {file:'packs/ice.json',      skin:'winter',   label:'冬・滑る氷', rule:'slide',
            dir:'ice', emoji:'⛄🧊', name:'すべる氷',
            desc:'氷は壁か氷にぶつかるまで滑ります'},
  mark:    {file:'packs/mark.json',     skin:'normal',   label:'印あわせ',   rule:'marks', man:'🐣',
            dir:'mark', emoji:'🐣📦', name:'印あわせ',
            desc:'AはAの置き場へ。印の無い荷物は印の無い置き場へ'},
  roll:    {file:'packs/roll.json',     skin:'beetle',   label:'フンころがし', rule:'roll',
            dir:'roll', emoji:'🪲💩', name:'ころがし',
            desc:'押すたびに90度まわります。向きを戻して置きましょう'},
  spring:  {file:'packs/spring.json',   skin:'spring',   label:'春・2匹',    rule:'duo',
            dir:'bee', emoji:'🐝🌸', name:'2匹のミツバチ',
            desc:'ミツバチは2匹。1回押すごとに交代します'},
  ant:     {file:'packs/ant.json',      skin:'ant',      label:'蟻・同僚',   rule:'ants',
            dir:'ant', emoji:'🐜📦', name:'はたらきアリ',
            desc:'同僚のアリも、あなたが押すたびに1つ押します'},
};
/* デバッグ表示のときに出す「ルールを選ぶ」の一覧。
   本編を先頭に、あとはパックの並び順。入口はどれもサイトの直下にあるので
   ../<入口>/ で行き来できる */
const MODES=[{dir:'warehouse', emoji:'🐥📦', name:'倉庫パズル',
              desc:'ふつうのルール。1面から順に進む', pack:null}]
  .concat(Object.keys(PACKS).map(k=>({dir:PACKS[k].dir, emoji:PACKS[k].emoji,
                                      name:PACKS[k].name, desc:PACKS[k].desc, pack:k})));
const PACK=(()=>{
...
const SKINS={
  ...
  ant:     {man:'🐜',  box:'📦', label:'蟻',           title:'はたらきアリ', icon:''},
};
...
/* 音を丸ごと切る。「動かせなくなる」のが音のせいかどうかを、実機で白黒つけるため。
   切ると AudioContext も <audio> も作らず、指やキーに音の下ごしらえを繋ぎもしない。
   ?sound=0 でも、デバッグ表示のボタン(音を止める)でも切り替えられる。端末に覚える */
let soundSaved=false;
try{ soundSaved = localStorage.getItem('warehouse-sound')==='0'; }catch(e){}
const SOUND = QS.get('sound')==='0' ? false : (QS.get('sound')==='1' ? true : !soundSaved);
/* 滑る動きを止める。「アニメーションで下が切れる/残る」が動きのせいかどうかを、
   実機で白黒つけるため。止めると絵は定位置にそのまま出る(遊べる)。
   ?anim=0 でも、デバッグ表示のボタン(動きを止める)でも切り替えられる。端末に覚える */
let animSaved=false;
try{ animSaved = localStorage.getItem('warehouse-anim')==='0'; }catch(e){}
const ANIM = QS.get('anim')==='0' ? false : (QS.get('anim')==='1' ? true : !animSaved);
...
if(RULE==='marks') document.body.classList.add('rule-marks');
if(RULE==='water') document.body.classList.add('rule-water');
if(RULE==='ants') document.body.classList.add('rule-ants');
if(RULE==='duo') document.body.classList.add('rule-duo');
// ステージを行き来できるモード
...
let actx = null;
let master = null;           // 全部の音をここへ通す。裏へ回ったら 0 にして、溜まった予約を聞こえないようにする
let audioMuted = false;
let audioGen = 0;            // 裏へ回るたびに増やす。遅延の setTimeout は古い世代を捨てる
let wavNo = 0;
let buzz = null;
...
let silentEl=null, mediaNo=0, pokeAt=0;
function pokeMedia(){
  // 断られ続ける端末で毎回叩くと、その端末が固まることがある(以前これで固めた)。
  // 3回続けて断られたら、表に戻るまで叩かない。
  // 数え直すのは表に戻ったとき(wakeAudio)だけ。アプリを往復すればまた試す。
  //
  // 気をつけること が2つある。どちらも iPhone の Safari で実際に効いた。
  // ・1回のタップで pointerdown / touchstart / touchend / click の4つが飛ぶ。
  //   間を置かずに叩くと、1タップで3〜4回も鳴らしに行く
  // ・断られたときだけ数えていると、その4つは最初の断りが返る前に
  //   全部すり抜ける(3回上限のはずが4回通った)。先に数えて、開いたら0に戻す
  const now=Date.now();
  if(now-pokeAt<300) return;                 // 同じタップの中では一度だけ
  pokeAt=now;
  if(mediaNo>=3) return;
  mediaNo++;
  try{
    ...
    if(p&&p.then) p.then(()=>{ audioLog.ses='開'; mediaNo=0; })
                   .catch(e=>{ audioLog.ses='×'+String(e&&e.name||'').slice(0,8); });
    else { audioLog.ses='開?'; mediaNo=0; }
  }catch(e){ audioLog.ses='×'; }
}
function getMaster(){
  if(!actx) return null;
...
function muteAudio(){
  // ここで断られた回数を数え直してはいけない。iOS は音が中断されるたびに
  // statechange を出し、その中でここへ来る。数え直すと、前面にいるまま
  // 歯止めが何度も外れて <audio> を叩き続けることになる(端末が固まる)。
  // 数え直しは表に戻ったとき(wakeAudio)だけにする
  audioGen++;
  audioMuted=true;
  try{ if(master) master.gain.setValueAtTime(0, actx?actx.currentTime:0); }catch(e){ try{ master.gain.value=0; }catch(e2){} }
  try{ if(buzz && buzz.g){ buzz.g.gain.cancelScheduledValues(actx?actx.currentTime:0); buzz.g.gain.value=0.0001; } }catch(e){}
  try{ if(silentEl) silentEl.pause(); }catch(e){}
}
function unmuteAudio(){
  if(!actx){ audioMuted=false; return; }
  if(!audioMuted && master && master.context===actx) return;  // もう開いている。差し替えると今鳴っている音が切れる
  audioMuted=false;
  try{
    // 止まっていたあいだに予約された音は、古い master に残っている。
    // 新しい管に差し替えると、溜まったぶんは聞こえない
    if(master){ try{ master.disconnect(); }catch(e){} }
    master=actx.createGain();
    master.gain.value=1;
    master.connect(actx.destination);
    if(buzz && buzz.g){
      try{ buzz.g.disconnect(); }catch(e){}
      buzz.g.connect(master);
    }
  }catch(e){}
}
function bindAudioState(){
  if(!actx || actx._bound) return;
  actx._bound=true;
  const onState=()=>{
    if(!actx || actx.state==='running') return;
    muteAudio();
  };
  try{ actx.addEventListener('statechange', onState); }catch(e){ actx.onstatechange=onState; }
}
function warmupAudio(){
  try{
    const m=getMaster(); if(!m) return;
    const b=actx.createBuffer(1,1,22050);
    const src=actx.createBufferSource();
    src.buffer=b; src.connect(m); src.start(0);
  }catch(e){}
}
/* touched=true は「いま指(かキー)の中にいる」という意味。
   操作の外で箱を作ると、iOS はあとから動かしてくれないことがある。
   起こす回数に上限を置かない。以前は8回で諦めて、アプリを往復したあと
   ずっと黙る原因になっていた */
function ensureAudio(touched){
  if(!SOUND) return;                          // 音を切っている
  if(!AC || !touched) return;                 // 操作の外では何もしない
  audioLog.touch++;
  try{
    if(!actx || actx.state==='closed'){
      actx=new AC(); master=null; buzz=null; noiseBuf=null; smoothBuf=null;
      audioLog.make++;
      bindAudioState();
    }
    if(actx.state==='running'){
      // もう開いている。ここで <audio> を叩く必要はない。
      // 叩いていたころは、音が正常に出ている状態でも
      // 1タップで3回・1打鍵で2回、鳴らしに行っていた
      audioLog.ok++;
      unmuteAudio();
      warmupAudio();
      return;
    }
    pokeMedia();                               // 開いていないときだけ。resume より先に叩く
    audioLog.resume++;
    const p=actx.resume();
    if(p&&p.then) p.then(()=>{
      if(actx && actx.state==='running'){ unmuteAudio(); warmupAudio(); }
    }).catch(e=>{ audioLog.err=String(e&&e.name||e).slice(0,12); });
  }catch(e){ audioLog.err=String(e&&e.name||e).slice(0,12); }
}
const audioTouch=()=>ensureAudio(true);
// 音を切っているときは、受け口そのものを繋がない。
// 切り分けのときに「音の下ごしらえが指の道に入っている」状態を残さないため
if(SOUND) for(const ev of ['touchstart','touchend','pointerdown','click','keydown']){
  document.addEventListener(ev, audioTouch, {passive:true, capture:true});
}
/* 裏へ回ったら黙る。起こすのは次の操作の中だけ。
   見えた瞬間に resume すると、iOS は拒否して、かつ止まっていたあいだの
   予約が一斉に鳴ることがある */
function sleepAudio(){ muteAudio(); }
function wakeAudio(){
  // 表に戻ったので、断られた回数を数え直す。
  // 「アプリを往復すればまた試す」はここで保つ。ずっと黙ったままにはならない
  mediaNo=0; wavNo=0;
  try{ if(actx && actx.state!=='running' && actx.state!=='closed') actx.resume().catch(()=>{}); }catch(e){}
}
document.addEventListener('visibilitychange',()=>{
  if(document.hidden) sleepAudio(); else wakeAudio();
});
window.addEventListener('pagehide', sleepAudio);
window.addEventListener('freeze', sleepAudio);
window.addEventListener('pageshow', wakeAudio);
function laterSound(ms, fn){
  const gen=audioGen;
  setTimeout(()=>{ if(gen!==audioGen) return; fn(); }, ms);
}
function beep(freq, dur, delay, type, vol){
  // 止まっている間に鳴らそうとすると、時計が進まないので予約が同じ時刻に溜まる。
  // 裏へ回っているあいだは捨てる。戻ってからの一斉鳴りを防ぐ
...
/* 音程を動かしながら鳴らす。水に入るポチャンのような、
   短い間に高さが変わる音に使う */
function swoop(from, to, dur, delay, type, vol){
  if(document.hidden) return;
  if(!actx || actx.state !== 'running'){
    if(audioLog.touch) playWav(from, dur, delay, type, vol);
    return;
  }
  const m=getMaster(); if(!m) return;
  const t = actx.currentTime + (delay||0);
...
function playWav(freq,dur,delay,type,vol){
  ...
  const gen=audioGen;
  const go=()=>{ try{
    if(gen!==audioGen || document.hidden) return;
    if(wavNo>=3) return;                       // 断られ続けるなら、もう鳴らそうとしない
    const a=wavFor(freq,dur,type,vol); a.currentTime=0;
    const p=a.play();
    if(p&&p.then) p.then(()=>{ wavNo=0; })
                   .catch(e=>{ wavNo++; audioLog.err='波:'+String(e&&e.name||e).slice(0,10); });
    audioLog.wav++;
...
const SFX_SETS={
  ...
  winter:{
    ...
    // 滑る時間(ミリ秒)を渡せる。遠くまで滑るときは、そのぶん長く鳴らす。
    // 形(前3.5割は同じ大きさ、残りで細くなる)は変えないので、音の性格は同じ
    push(ms){ noise(Math.max(0.12,(ms||110)/1000), 0,
                    {type:'bandpass', from:3400, q:5, vol:0.04, hold:0.35, smooth:true}); },
    blocked(){ noise(0.1, 0, {from:400, to:180, q:1.2, vol:0.05}); },
    // カチッ。壁か氷にぶつかって止まる
    hit(){
      beep(2100, 0.03, 0, 'square', 0.055);
      beep(1400, 0.04, 0.012, 'triangle', 0.03);
      noise(0.03, 0, {from:2800, to:900, q:1.2, vol:0.022});
    },
    // シャリーン。高い音を少しずらして重ねる
    onGoal(){
  ...
  summer:{
    ...
    // ポチャン。水に入る音。泡の共鳴は上がっていくので、音程を上げながら短く。
    // そこへ飛沫のシュッをひと粒重ねる
    splash(){
      swoop(360, 1150, 0.13, 0, 'sine', 0.09);
      noise(0.10, 0.01, {from:2800, to:1100, q:1.1, vol:0.028});
    },
...
function parseBoard(board){
  ...
  for(let y=0;y<h;y++){
    ...
    for(let x=0;x<w;x++){
      ...
      // 印あわせ。印つきの荷物は 1〜9、印つきの置き場は a〜i。
      // 印の合う置き場でないとクリアにならない。印のない $ と . も混ぜられる
      const ni=NUMS.indexOf(c), gi=GOALS.indexOf(c);
      if(ni>=0){ num[ni]=i; continue; }
      if(gi>=0){ slot[gi]=i; continue; }
      if(c==='$'||c==='*') free.push(i);
      if(c==='.'||c==='*'||c==='+') open.push(i);
      if(c==='&'){ ants.push(i); continue; }   // 蟻。同僚のアリ
      if(c==='@'||c==='+'){ player=i; players.push(i); }   // 春は2匹いるので全部拾う
    }
  }
  // 春は、盤に書いた順(左上から)で1匹目・2匹目。1匹目から始める
  if(players.length>1) player=players[0];
  // 印あわせは、荷物の並び順が「どの印か」を表すので並べ替えない。
  // 先頭から印つき(A B C…)、そのあとに印のない荷物が続く
  return num.length
    ? {grid, water, w, h,
       boxes:num.concat(free), goals:slot.concat(open),
       num:num.slice(), slot:slot.slice(), open:open.slice(), nmark:num.length,
       players, ants, player}
    : {grid, water, w, h, boxes:free.sort((a,b)=>a-b), goals:open.sort((a,b)=>a-b),
       players, ants, player};
}
...
(function(){
  // 印あわせは number から mark に名前を変えた。前の名前の記録が残っていれば引き継ぐ
  if(PACK==='mark' && store.get(PKEY)===null){
    const old=store.get('progress-number');
    if(old) store.set(PKEY, old);
  }
...
let rot=[];                // フンコロガシ: 荷物ごとの向き(0〜3)
let bees=[], turn=0;       // 春: ミツバチ全員の居場所と、いま何番の子の番か。
                           // 番でない子は壁になる。player は bees[turn] と同じ
const waitBee=n=>{ for(let i=0;i<bees.length;i++) if(i!==turn && bees[i]===n) return true; return false; };
let coants=[];             // 蟻: 同僚。{at:居場所, claim:取り付いた荷物, dir:押す向き(0上1下2左3右)}
let antBusy=false, antSeq=0;  // 自分の押しが終わってから同僚を動かす間、入力を止める
let antTaboo=-1;              // 蟻: 主人公が直前に押した荷物。同僚は触らない
function cancelAnts(){ antSeq++; antBusy=false; }
/* リス: いま穴へ落ちている最中のドングリ。
   盤の中身は押した時点でもう埋まった扱いにするが、絵だけは、
   ドングリが穴まで動き終わるまで「まだ乗っている」ように描く */
let dropAt=-1, dropSeq=0, glideSeq=0;
function dropDone(){ dropAt=-1; dropSeq++; glideSeq++; }
...
/* 絵が滑っている間は、盤の大きさを変えてはいけない。
   slide() は動き始めのマス幅で移動量を決めているので、途中で --cell が変わると
   移動量と食い違い、iPhone では下の行が切れたり、前の絵が残ったりする。
   お知らせの出し入れ(showMsg / hideMsg)でも layoutBoard を呼ぶので、
   デバッグ表示では毎手これが起きていた(実測 40px→34px) */
let slideUntil=0, layoutWait=null;
function holdLayout(ms){ slideUntil=Math.max(slideUntil, Date.now()+(ms||SLIDE_MS)+40); }
function layoutBoard(){
  const left=slideUntil-Date.now();
  if(left>0){                                      // 動いている。終わってからにする
    clearTimeout(layoutWait);
    layoutWait=setTimeout(layoutBoard, left);
    return;
  }
  alignHeaderButtons();
  alignPushes();
  ...
  // 盤の大きさが変わったら控える。遊んでいる最中に変わっていないかを見るため
  if(typeof diag!=='undefined' && cell!==lastCell){
    diag.盤.push(new Date().toTimeString().slice(0,8)+' '+(lastCell||'-')+'→'+cell+'px');
    if(diag.盤.length>12) diag.盤.shift();
  }
  lastCell=cell;
...
let tableFails=0;        // Worker がしくじった回数。一度で見限らない
let tableOff=false;      // この面では表を作れなかった(ヒントと詰み判定は出さない)
let tableMs=0, tableHow='未作成';   // 「止まった記録」に出す
...
function ensureWorker(){
  ...
  try{
    ...
    tableWorker.onmessage=e=>{
      ...
      tableOff=!dist;
      tableMs=Date.now()-tableAt; tableHow='Worker';
      tableReady();
    };
    /* Worker がしくじったとき。**一度で見限ってはいけない。**
       前はここで false に落としていたので、一度でも失敗すると以後ずっと
       主線(画面を描く線)で表を作り続け、面を開くたびに画面ごと止まっていた。
       作り直してみて、それでもだめなときだけ諦める */
    tableWorker.onerror=()=>{
      try{ tableWorker.terminate(); }catch(e){}
      tableWorker = (++tableFails>=3) ? false : null;
      requestTable();
    };
...
function requestTable(){
  ensureWorker();
  const id=++tableReq;
  dist=null; tableOff=false; tableAt=Date.now();
  if(tableWorker){
    tableHow='Worker';
    tableWorker.postMessage({id, grid:puzzle.grid, w:puzzle.w, goals:puzzle.goals, cap:3000000});
    return;
  }
  /* Worker が使えない環境。ここは画面と同じ線なので、**必ず時間を切る**。
     切らないと、表ができるまで画面ごと止まって何も受け付けなくなる
     (第800面で実測 5.1秒。iPhone ならその数倍)。
     小さい盤は 150ms で足りるので、そこまでは今までどおりヒントが出る */
  showMsg('詰み判定の表を作っています…');
  setTimeout(()=>{
    if(id!==tableReq) return;                       // その間に面が変わったらやめる
    const t0=Date.now();
    dist=solvableStates(puzzle.grid, puzzle.w, puzzle.goals, 3000000, t0+150);
    tableOff=!dist;
    tableMs=Date.now()-t0; tableHow='主線(Workerなし)';
    hideMsg();
    tableReady();
  },0);
}
...
function startLevel(i){
  ...
  rot=boxes.map(()=>0);        // フンコロガシ。最初はどれも向き0
  bees = RULE==='duo' && puzzle.players ? puzzle.players.slice() : [];
  turn = 0;   // 春。2匹目
  coants = puzzle.ants ? puzzle.ants.map(a=>({at:a, claim:-1, dir:-1})) : [];   // 蟻。同僚
  antTaboo=-1;
  dropDone();
...
function buildBoard(){
  ...
  for(let i=0;i<puzzle.grid.length;i++){
    ...
    fig.className='fig';
    d.appendChild(fig);
    // 印あわせの、置き場の印を置く台。絵とまったく同じ大きさ・同じ場所で、
    // ただし動かない。絵の後ろに敷くので、ヒヨコや荷物が来ると隠れる
    if(RULE==='marks'){
      const base=document.createElement('span');
      base.className='tagbase';
      d.appendChild(base);
      d._base=base;
    }
...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    el.querySelectorAll('.antaim').forEach(n=>n.remove());
    const wall=puzzle.grid[i], goal=goalSet.has(i);
    // 落ちている最中のドングリは、盤の上ではもう消えているが、絵はまだ描く
    const box=boxSet.has(i) || i===dropAt;
    const outside=wall&&outsideMask&&outsideMask[i];
    const wet=puzzle.water && puzzle.water[i];
    const hole = RULE==='holes' && goal;
    const shut = hole && filled.has(i) && i!==dropAt;   // 埋まるのは動き終わってから
    el.className='cell'+(outside?' void':wall?' wall':'')+(wet?' water':'')
      +(hole ? (shut?' shut':' hole') : (goal&&!box?' goal':''))
      +(box?' box':'')+(box&&goal&&!hole?' on':'');
    // 春はミツバチが2匹。番のほうを光らせ、待っているほうは薄くする
    const waiting = RULE==='duo' && waitBee(i);
    const ant = RULE==='ants' ? coants.find(a=>a.at===i) : null;
    const coworker = !!ant;
    fig.textContent = box ? SKIN.box : (i===player||waiting||coworker ? SKIN.man : '');
    if(RULE==='ants'){
      el.classList.toggle('mate', coworker);
    }
    if(RULE==='duo'){
      el.classList.toggle('turn', i===player && !box);
      el.classList.toggle('wait', waiting);
    }
    // フンコロガシ。押した回数だけ時計回りに回して見せる
    if(RULE==='roll'){
      const k=boxes.indexOf(i);
      fig.style.rotate = (box && k>=0) ? (rot[k]*90)+'deg' : '';
      fig.style.left='0px'; fig.style.top='0px';   // 前の転がりの残りを消す
    }
    // 印あわせは、荷物と置き場に印(A B C…)を添える。
    // 荷物の印は絵の中(.fig)。押したとき絵と一緒に動く。
    // 置き場の印は動かない台(.tagbase)の上に置く。台は絵とまったく同じ
    // 大きさ・同じ場所に敷いてあるので見た目は揃い、絵より後ろなので、
    // ヒヨコや荷物がそのマスに来ると、その後ろに隠れる
    if(RULE==='marks'){
      const nm=puzzle.nmark|0;
      const bn=boxes.indexOf(i), gn=puzzle.slot.indexOf(i);
      const markedBox = bn>=0 && bn<nm;            // 印のない荷物は札を出さない
      if(markedBox){
        const t=document.createElement('span');
        t.className='tag';
        t.textContent=MARKS[bn];
        fig.appendChild(t);
      }
      if(el._base){
        el._base.textContent='';
        if(gn>=0 && bn<0){                  // 荷物が乗っているなら荷物の印のほうを出す
          const t=document.createElement('span');
          t.className='tag goaltag';
          t.textContent=MARKS[gn];
          el._base.appendChild(t);
        }
      }
      // A は A、B は B。印の無い荷物は印の無い置き場。違うところに乗せると赤
      if(bn>=0 && goal){
        const ok = markedBox ? gn===bn : gn<0;
...
function updateStats(){
  ...
  if(DEBUG && puzzle){
    ...
    $('boardNote').textContent='最短'+lv.p+'手'
      ...
      +' / 自己ベスト'+(b==null?'--':b+'手')
      +(RULE==='plain' ? ' / 表'+(dist?dist.size+'状態':(tableOff?'作れず':'計算中…')) : '')
...
function move(name, fromReplay){
  if(!puzzle||finished||EDIT||antBusy){
    // 何で弾いたかを控える。「動かせない」ときに、掛け金なのか別の理由なのかを分ける
    diagBlock(!puzzle?'盤がない' : finished?'クリア済み(finished)'
              : EDIT?'編集中(EDIT)' : '蟻の演出中(antBusy)');
    return;
  }
  ...
  dropDone();      // 前のドングリが落ちきる前に次の手が来たら、そこで埋める
  const d=dirOffset(name);
  const next=player+d, beyond=player+2*d;
  if(puzzle.grid[next]){ SFX.blocked(); return; }
  // 春。番でないミツバチは壁。すり抜けられない
  if(RULE==='duo' && waitBee(next)){ SFX.blocked(); return; }
  // 蟻。同僚のいるマスには入れない
  if(RULE==='ants' && coants.some(a=>a.at===next)){ SFX.blocked(); return; }
  ...
  if(bi>=0){
    ...
    // 春。相方には押し込めない
    if(RULE==='duo' && waitBee(beyond)){ SFX.blocked(); return; }
    // 蟻。同僚には押し込めない
    if(RULE==='ants' && coants.some(a=>a.at===beyond)){ SFX.blocked(); return; }
    ...
    undoStack.push({player, bees:bees.slice(), turn, ants:coants.map(a=>({at:a.at,claim:a.claim,dir:a.dir})), taboo:antTaboo, boxes:boxes.slice(), pushCount, moveCount, filled:new Set(filled), rot:rot.slice()});
    ...
    if(drop){
      boxes.splice(bi,1); rot.splice(bi,1); filled.add(dest);   // 穴に落ちて埋まる
      // 絵のうえでは、ドングリが穴まで動き終わってから埋める
      dropAt=dest;
      const seq=++dropSeq;
      setTimeout(()=>{ if(dropSeq===seq){ dropAt=-1; paint(); } }, SLIDE_MS);
    }
    else boxes[bi]=dest;
    const wasRot = RULE==='roll' ? rot[bi] : 0;         // 回る前の向き(演出に要る)
    if(RULE==='roll') rot[bi]=(rot[bi]+1)%4;            // 押すたび時計回りに90度
    // 冬は、押しても押した先のマスに入らない。その場に留まる。
    // (空いたマスへはいつでも歩いて入れるので、自機の行ける範囲は変わらない。
    //  つまり最短の押し手数は他のモードと同じで、面の記録もそのまま使える)
    // 冬は、押しても押した先のマスに入らない。滑るルールでも同じ。
    // (氷がどいたマスへはいつでも歩いて入れるので、自機の行ける範囲は変わらない。
    //  つまり最短の押し手数は変わらず、面の記録もそのまま使える)
    if(skinName!=='winter') player=next;
    // いま動いたほう。演出はこの位置に出す
    const pusher=player;
    // 春。1回押すごとに交代する。押したミツバチはその場で壁になり、相方が動きだす
    // 春。押した子は荷物のいた場所に残って壁になり、並び順で次の子の番になる
    if(RULE==='duo' && bees.length){
      bees[turn]=pusher;
      turn=(turn+1)%bees.length;
      player=bees[turn];
    }
    // 蟻。再生や検算では今までどおり同時に動かす(手数がずれない)。
    // 自分で遊ぶときは、自分の押しが滑り終わってから同僚を動かす。
    // 同時だと、どの荷物を自分が押したのか分からない
    const delayAnts = RULE==='ants' && !fromReplay;
    // 蟻。取り付いていた荷物が動いたら、執着先の位置も動かす。
    // いま自分が押した荷物には、同僚は触らない
    if(RULE==='ants'){
      for(const a2 of coants) if(a2.claim===next) a2.claim=dest;
      antTaboo=dest;
    }
    const acts = (RULE==='ants' && !delayAnts) ? runAnts(antTaboo) : null;
    pushCount++; moveCount++;
    paint();
    if(skinName==='winter'){
      const n=RULE==='slide' ? (dest-beyond)/d + 1 : 1;
      glide(dest, d, n);                     // ぶつかるまで滑る
      flashGlideGoals(next, dest, d, n, lastGlide);
      sweat(pusher, 0);                      // 自機はその場に留まる
    }else if(RULE==='roll'){
      roll(dest, d, wasRot*90, rot[bi]*90);  // まず回って、それから転がって移る
      slide(pusher, d);
      sweat(pusher, d);
    }else{
      slide(dest, d); slide(pusher, d);      // 荷物と自分が、いた場所から滑ってくる
      sweat(pusher, d);
    }
    // 蟻。同僚が動いたぶんも滑らせる(検算・再生のときだけ。ふだんは playAnts が出す)
    if(acts) for(const a of acts){ slide(a.to, a.dir); slide(a.at, a.dir); }
    if(coop){ coBox=dest; coMoved=true; }                  // 次はこの荷物だけ動かせる
    if(skinName==='winter'){
      // 滑っている間ずっとシューと鳴らし、止まってからカチッ。
      // 置き場に止まったら、そのあとシャリーン
      SFX.push(lastGlide);
      laterSound(lastGlide, ()=>{
        (SFX.hit||SFX.blocked)();
        if(goalSet.has(dest)){ SFX.onGoal(); bump(dest); }
      });
    }
    else if(goalSet.has(dest)){
      // マスに着いてから鳴らす。押し始めた瞬間だと、まだ途中なのに入ったように聞こえる
      laterSound(SLIDE_MS, ()=>{ SFX.onGoal(); bump(dest); });
    }
    else SFX.push();
    if(delayAnts){
      antBusy=true;
    ...
    undoStack.push({player, bees:bees.slice(), turn, ants:coants.map(a=>({at:a.at,claim:a.claim,dir:a.dir})), taboo:antTaboo, boxes:boxes.slice(), pushCount, moveCount});
    player=next;
    moveCount++;
    paint(); slide(player, d);
    // 夏。水のマスに入ったら、しずくを跳ねさせてポチャンと鳴らす
    if(puzzle.water && puzzle.water[next]){
      splash(next, d);
      (SFX.splash||SFX.step)();
    }else SFX.step();
  }
...
const ANT_STEP_MS=280;                    // 同僚が1マス歩く時間。速いとどこへ行ったか分からない
const ANT_PUSH_MS=360;                    // 同僚が荷物を押す時間。自分の押しより長くする
const GLIDE_MS=110;                       // 冬の氷。1マスぶん滑るときの時間。ふつうの押しと同じ
/* 距離が伸びてもあまり待たせない。1マスは0.11秒、遠いときも0.28秒まで */
const glideMs=n=>Math.min(280, Math.round(GLIDE_MS*Math.pow(Math.max(1,n||1), 0.35)));
let lastGlide=GLIDE_MS;                   // 直前に滑った時間。次の演出を待たせるのに使う
const stepMs=()=>skinName==='winter'?lastGlide:SLIDE_MS;
...
function glide(i, d, n){
  ...
  const ms=glideMs(n);
  lastGlide=ms;
  holdLayout(ms);                          // 滑っている間は盤の大きさを変えない
  ...
  fig.style.transitionDuration=ms+'ms';   // 距離に合わせて伸ばす
  fig.style.transform='';
  setTimeout(()=>{ fig.classList.remove('glide'); fig.style.transitionDuration=''; }, ms+40);
}
/* 滑っている間、通過する置き場もいちおう「収まった」色にする。
   盤の中身はもう行き先に氷があるが、絵はまだ出発点にいるので、
   色だけ絵に合わせて動かす。着いた置き場は、着いてから「収まった」色 */
function flashGlideGoals(from, dest, d, n, ms){
  const seq=++glideSeq;
  if(goalSet.has(dest) && cellEls[dest]){
    cellEls[dest].classList.remove('on');
    cellEls[dest].classList.add('goal');
  }
  const cells=[];
  for(let i=from; i!==dest+d; i+=d) cells.push(i);
  const steps=Math.max(1, n||1);
  cells.forEach((cell,k)=>{
    if(!goalSet.has(cell) || !cellEls[cell]) return;
    if(cell===dest){
      setTimeout(()=>{
        if(seq!==glideSeq) return;
        cellEls[dest] && cellEls[dest].classList.add('on');
      }, ms);
      return;
    }
    const enter=Math.max(0, Math.round((k-0.3)/steps*ms));
    const leave=Math.round((k+0.7)/steps*ms);
    const light=()=>{
      if(seq!==glideSeq) return;
      cellEls[cell] && cellEls[cell].classList.add('on');
    };
    if(enter===0) light(); else setTimeout(light, enter);
    setTimeout(()=>{
      if(seq!==glideSeq) return;
      cellEls[cell] && cellEls[cell].classList.remove('on');
    }, Math.max(enter+50, leave));
  });
}
// 移動したマスの絵を、1マスぶん手前に置いてから定位置へ戻す
function slide(i, d, ms){
  const fig=cellEls[i] && cellEls[i].firstChild;
  if(!fig) return;
  if(!ANIM) return;                                 // 動きを止めている
  holdLayout(ms);                                   // 滑っている間は盤の大きさを変えない
  ...
  fig.classList.remove('sliding');
  fig.style.transitionDuration=(ms||SLIDE_MS)+'ms';
  fig.style.transform='';
}
/* フンコロガシ。転がりながら隣のマスへ移る。回るのと移るのは同時。
   移動には transform ではなく left/top を使う。
   CSS は rotate プロパティを transform より先に掛けるので、同じ要素で
   両方を使うと、傾いたぶんだけ移動の向きまで回ってしまう
   (右へ押したのに下から入ってくる)。left/top なら向きと位置が混ざらない。
   left は auto とは繋がらないので、戻すときは空文字ではなく 0px を書く */
function roll(i, d, from, to){
  const fig=cellEls[i] && cellEls[i].firstChild;
  if(!fig) return;
  if(!ANIM) return;                                 // 動きを止めている
  holdLayout();                            // 転がっている間は盤の大きさを変えない
  const step=cellEls[i].getBoundingClientRect().width + 2;
  const dx=(d===1?-1:d===-1?1:0)*step;
  const dy=(d===puzzle.w?-1:d===-puzzle.w?1:0)*step;
  fig.classList.add('sliding');           // まず、回る前の向きで元のマスに置く
  fig.style.rotate=from+'deg';
  fig.style.left=dx+'px'; fig.style.top=dy+'px';
  void fig.offsetWidth;                   // ここで一度置き直させる
  fig.classList.remove('sliding');
  fig.style.rotate=to+'deg';              // 回りながら
  fig.style.left='0px'; fig.style.top='0px';   // 同時に移る
}
...
/* 夏。カニが水のマスに入ったとき、しずくを左右へはねさせる。
   粒は絵文字にせず CSS で尖らせる。丸いと回っているのが分からない。
   カニと同じマスで同じだけ滑らせる。着水してから飛ぶと、入った感じが出ない */
function splash(i, d){
  const el=cellEls[i];
  if(!el) return;
  const wrap=document.createElement('span');
  ...
  wrap.style.transition='transform .11s ease-out';
  wrap.style.transform='';
  setTimeout(()=>{ if(wrap.parentNode) wrap.remove(); }, 760);
}
/* 蟻。こちらが1つ押すと、同僚も1匹ずつ押す。
   規則は warehouse/rules.js の ants と同じにしてある(片方だけ直さないこと):
     ・押すのは自分から一番近いダンボール
     ・向きは歩いてきた向きのまま(回り込まない)
     ・押せないときは動かない
     ・盤の左上に近い蟻から順に、前の蟻が動いた結果を見て動く
   返すのは、動いた蟻の記録。演出に使う */
/* 蟻を1匹だけ進める。規則は warehouse/rules.js の ants と同じにしてある
   (片方だけ直さないこと。画面と探索で答えが食い違う)。
     ・取り付いていなければ、一番近い荷物に取り付く(押す位置までの歩数)
     ・荷物に接するまでの道は、曲がる回数がいちばん少ないものを選ぶ。
       接したときに進んでいた向きが、そのまま押す向きになる
     ・取り付いたら、その荷物に執着する
     ・いまの向きに押せなければ、左右で曲がりの少ないほう、だめなら反対側へ回り込む
     ・1手につき1マス押す。押す位置までは何マスでも歩く(手数には数えない)
     ・主人公が直前に押した荷物(taboo)には触らない
   盤の中身(boxArr / antArr)はその場で書き換え、何をしたかを返す */
function antStep(who, boxArr, antArr, mePlayer, taboo, dry){
  const {grid,w}=puzzle;
  const D=[-w,w,-1,1];
  const me=antArr[who];
  const blocked=new Set(boxArr);
  antArr.forEach((a,i)=>{ if(i!==who) blocked.add(a.at); });
  blocked.add(mePlayer);
  const walk=from=>{
    ...
    for(let i=0;i<q.length;i++){
      ...
      for(const s of D){
        const n=c+s;
        if(n<0||n>=grid.length||grid[n]||blocked.has(n)||dist.has(n)) continue;
  ...
  if(me.claim>=0 && (boxArr.indexOf(me.claim)<0 || me.claim===taboo || goalSet.has(me.claim))){
    me.claim=-1; me.dir=-1;
  }
  const dist=walk(me.at);
  // 曲がる回数がいちばん少ない道。まっすぐ0、向きを変えて1。最初の1歩は0。
  // 向きごとに数えるが、押す向きとは切り離してある(bestArrival でまとめる)
  const bends=from=>{
    const N=grid.length;
    const cost=new Int32Array(N*4).fill(-1);
    const prev=new Int32Array(N*4).fill(-1);
    const dq=new Array(N*8); let head=N*4, tail=N*4;
    for(let k=0;k<4;k++){
      const n=from+D[k];
    ...
    while(head<tail){
      const id=dq[head++], c=id>>2, k=id&3, t=cost[id];
      for(let j=0;j<4;j++){
        const n=c+D[j];
        if(n<0||n>=N||grid[n]||blocked.has(n)) continue;
  ...
  const bend=bends(me.at);
  /* そのマスに着くまでの曲がり回数と、その着き方。着いたときの向きは問わない。
     ...
     探索側(rules.js の bendTo)と同じ数え方でないと、画面と答えが食い違う */
  const bestArrival=cell=>{
    let t=-1, q0=-1;
    for(let q=0;q<4;q++){ const c=bend.cost[cell*4+q]; if(c>=0 && (t<0||c<t)){ t=c; q0=q; } }
    return {t, q:q0};
  };
  const ok=(b,k)=>{
    ...
    const t=bestArrival(stand).t;                      // 着いたときの向きは問わない
    if(t<0) return -1;
    return t*1000 + dist.get(stand);                   // 曲がり優先、同じなら短いほう
  };
  const taken=new Set();
  antArr.forEach((a,i)=>{ if(i!==who && a.claim>=0) taken.add(a.claim); });
  if(me.claim<0){
    let best=null;
    for(const b of boxArr.slice().sort((x,y)=>x-y)){
      if(b===taboo || taken.has(b) || goalSet.has(b)) continue;  // 収まった荷物は触らない
  ...
  const b=me.claim;
  if(ok(b,me.dir)<0){
    const side = (me.dir===0||me.dir===1) ? [2,3] : [0,1];
    let pick=null;
    for(const k of side){ const t=ok(b,k); if(t>=0 && (!pick || t<pick.t)) pick={k,t}; }
    if(!pick){ const t=ok(b, me.dir^1); if(t>=0) pick={k:me.dir^1,t}; }
    if(!pick){ me.claim=-1; me.dir=-1; return null; }
    me.dir=pick.k;
  }
  // 押す位置まで歩いて、1マス押す。歩く道のりは見せるだけで、手数には数えない
  const dir=D[me.dir], stand=b-dir, to=b+dir;
  // 見せる道も、選んだ「曲がりの少ない道」をそのままたどる
  const path=[];
  {
    const arr=bestArrival(stand);          // 曲がりのいちばん少ない着き方をたどる
    let id=stand*4+arr.q;
    if(stand===me.at || arr.q<0) path.push(me.at, stand);
    else {
      while(id>=0){ path.push(id>>2); id=bend.prev[id]; }
      path.push(me.at);
      path.reverse();
    }
  }
  // dry のときは盤を動かさない。演出で「歩いてから押す」を見せるため、
  // 押す中身は呼んだ側があとから当てる(先に動かすと、歩く前に荷物が飛ぶ)
  if(!dry){
    boxArr[boxArr.indexOf(b)]=to;
    me.at=b; me.claim=to;
  }
  return {from:stand, at:b, box:b, dir, to, path};
}
function runAnts(taboo){
  const acts=[];
  for(let who=0; who<coants.length; who++){
    const a=antStep(who, boxes, coants, player, taboo);
    if(a) acts.push(a);
  }
  // 次の手は「盤の左上に近い蟻から」。動いたあとに並べ直しておく
  // (探索側も同じ。ここを忘れると、何手か進んでから画面と答えが食い違う)
  coants.sort((x,y)=>x.at-y.at);
  return acts;
}
function antWait(ms, seq){
  return new Promise(ok=>setTimeout(()=>ok(seq===antSeq && !finished), ms));
}
function antDirWord(dir){
  const w=puzzle.w;
  return dir===-w?'上':dir===w?'下':dir===-1?'左':'右';
}
/* 同僚を1匹ずつ、歩いてきた道ごと見せる。同時だと誰が何をしたか分からない */
/* 同僚を1匹ずつ見せる。歩く道のりは演出で、手数には数えない */
async function playAnts(seq, done){
  const live=()=>seq===antSeq && !finished;
  if(!await antWait(SLIDE_MS+80, seq)){ if(live()&&done) done(); return; }
  const taboo=antTaboo;
  for(let who=0; who<coants.length; who++){
    if(!live()) return;                       // 新しい手が始まった。こちらは黙って降りる
    const plan=antStep(who, boxes, coants, player, taboo, true);   // 決めるだけ
    if(!plan) continue;
    showMsg('同僚が '+SKIN.box+' を'+antDirWord(plan.dir)+'へ押します');
    // 押す位置まで歩いて見せる。ここではまだ盤を動かさない
    const path=plan.path||[];
    for(let i=1;i<path.length;i++){
      if(path[i]===path[i-1]) continue;
      if(!live()) return;
      coants[who].at=path[i];
      paint();
      slide(path[i], path[i]-path[i-1], ANT_STEP_MS);
      if(!await antWait(ANT_STEP_MS+40, seq)) return;
    }
    if(!live()) return;
    // ここで実際に押す
    boxes[boxes.indexOf(plan.box)]=plan.to;
    coants[who].at=plan.at;
    coants[who].claim=plan.to;
    paint();
    slide(plan.to, plan.dir, ANT_PUSH_MS);
    slide(plan.at, plan.dir, ANT_PUSH_MS);
    if(!await antWait(ANT_PUSH_MS+80, seq)) return;
  }
  if(!live()) return;
  coants.sort((x,y)=>x.at-y.at);              // 次の手は左上に近い蟻から(runAnts と同じ)
  paint();
  hideMsg();
  if(done) done();
}
function bump(i){
  const el=cellEls[i];
  if(!el) return;          // 遅れて届いたとき、もう別の面に変わっていることがある
  el.classList.add('bump');
  setTimeout(()=>el.classList.remove('bump'),200);
}
function undo(){
  if(!undoStack.length||finished||coop||antBusy) return;
  effort.u++;
  const s=undoStack.pop();
  dropDone();
  player=s.player; boxes=s.boxes; pushCount=s.pushCount; moveCount=s.moveCount;
  if(s.bees){ bees=s.bees; turn=s.turn|0; }
  if(s.ants) coants=s.ants;
  if(s.taboo!==undefined) antTaboo=s.taboo;
...
function resetLevel(){
  ...
  bees = RULE==='duo' && puzzle.players ? puzzle.players.slice() : [];
  turn = 0;
  coants = puzzle.ants ? puzzle.ants.map(a=>({at:a, claim:-1, dir:-1})) : [];
  antTaboo=-1;
  dropDone();
  undoStack=[]; pushCount=0; moveCount=0;
  deadShown=false;
...
function hint(){
  ...
  if(tableOff){                                // 表が作れなかった面。待たせない
    showMsg('この面は大きすぎて、ヒントの表を作れませんでした。', true);
    return;
  }
...
let msgTimer=null;
/* お知らせは盤の上に重ねてあるので、出しても消しても場所は変わらない。
   layoutBoard() を呼ぶ必要はない(呼ぶと盤の大きさが変わってしまう) */
...
function checkWin(){
  ...
  // 印あわせ。A は A の置き場、B は B の置き場。印の無い荷物は印の無い置き場へ
  else if(RULE==='marks'){
    const nm=puzzle.nmark|0;
    const open=new Set(puzzle.open||[]);
    for(let k=0;k<boxes.length;k++){
      const b=boxes[k];
      if(k<nm){ if(puzzle.slot[k]!==b) return false; }
      else if(!open.has(b)) return false;
    }
  }
  // フンコロガシは、置き場に乗っていても向きが戻っていないとだめ
  else if(RULE==='roll'){ if(!boxes.every((b,k)=>goalSet.has(b) && rot[k]===0)) return false; }
  ...
  // ファンファーレも同じ。押し始めた瞬間に鳴らすと、まだ途中なのにクリアしたように聞こえる
  const winGen=audioGen;
  setTimeout(()=>{
    if(winGen===audioGen) SFX.win();
...
function boardToXSB(){
  ...
  for(let y=0;y<puzzle.h;y++){
    ...
    for(let x=0;x<puzzle.w;x++){
      ...
      // 印と水は文字そのものが種類なので、先に書く。あとに回すと $ や . になって消える
      if(RULE==='marks'){
        const n=(puzzle.num||[]).indexOf(i);
        if(n>=0){ r+=NUMS[n]; continue; }
        const g=(puzzle.slot||[]).indexOf(i);
        if(g>=0){ r+=GOALS[g]; continue; }
      }
      // カニが水の上にいるとき、@ だけ書くと水が消える。
      // その盤で「解けるか」を見ると、荷物が足元へ押し込めることになって狂う
      if(puzzle.water && puzzle.water[i] && i===player){ r+='!'; continue; }
      if(puzzle.water && puzzle.water[i] && !boxSet.has(i) && !goalSet.has(i)){
        r+='~'; continue;
      }
      // 蟻の同僚と、春の2匹目。書き落とすと「解けるか確かめる」が別の盤を見てしまう
      if(RULE==='ants' && coants.some(a=>a.at===i)){ r+='&'; continue; }
      if(RULE==='duo' && waitBee(i)){ r+='@'; continue; }
      const g=goalSet.has(i), b=boxSet.has(i), m=(i===player);
      r += b ? (g?'*':'$') : m ? (g?'+':'@') : g ? '.' : (puzzle.water&&puzzle.water[i]?'~':' ');
    }
    rows.push(r);
  }
  return rows.join('/');
...
function vacate(i){
  ...
  if(puzzle.water) puzzle.water[i]=0;
  const bi=boxes.indexOf(i);
  if(bi>=0) boxes.splice(bi,1);
  goalSet.delete(i);
  if(RULE==='marks'){
    if(!puzzle.num) puzzle.num=[];
    if(!puzzle.slot) puzzle.slot=[];
    for(let k=0;k<puzzle.num.length;k++) if(puzzle.num[k]===i) puzzle.num[k]=undefined;
    for(let k=0;k<puzzle.slot.length;k++) if(puzzle.slot[k]===i) puzzle.slot[k]=undefined;
  }
}
function applyPieces(){
  if(RULE==='marks'){
    puzzle.num=(puzzle.num||[]).filter(x=>x!==undefined && x>=0);
    puzzle.slot=(puzzle.slot||[]).filter(x=>x!==undefined && x>=0);
    puzzle.nmark=puzzle.num.length;
    const marked=new Set(puzzle.num);
    const free=boxes.filter(b=>!marked.has(b));
    boxes=puzzle.num.concat(free);
    puzzle.free=free;
    puzzle.open=[...goalSet].filter(g=>puzzle.slot.indexOf(g)<0).sort((a,b)=>a-b);
    puzzle.boxes=boxes.slice();
    puzzle.goals=puzzle.slot.concat(puzzle.open);
    goalSet=new Set(puzzle.goals);
    return;
  }
  puzzle.boxes=boxes.slice();
...
function putMarkBox(k, i){
  const old=puzzle.num && puzzle.num[k];
  vacate(i);
  if(old!==undefined && old!==i){
    const bi=boxes.indexOf(old); if(bi>=0) boxes.splice(bi,1);
  }
  if(!puzzle.num) puzzle.num=[];
  puzzle.num[k]=i;
  if(boxes.indexOf(i)<0) boxes.push(i);
}
function putMarkGoal(k, i){
  const old=puzzle.slot && puzzle.slot[k];
  vacate(i);
  if(old!==undefined && old!==i) goalSet.delete(old);
  if(!puzzle.slot) puzzle.slot=[];
  puzzle.slot[k]=i;
  goalSet.add(i);
}
function editCell(i){
  ...
  if(tool==='wall'){
    ...
    puzzle.grid[i]=0;
    boxes.push(i);
  }else if(tool==='box1'){ puzzle.grid[i]=0; putMarkBox(0,i); }
  else if(tool==='box2'){
    if(!(puzzle.num&&puzzle.num[0]>=0)){ showMsg('先に荷物Aを置いてください', true); return; }
    puzzle.grid[i]=0; putMarkBox(1,i);
  }else if(tool==='box3'){
    if(!(puzzle.num&&puzzle.num[1]>=0)){ showMsg('先に荷物Bを置いてください', true); return; }
    puzzle.grid[i]=0; putMarkBox(2,i);
  }else if(tool==='slot3'){
    if(!(puzzle.slot&&puzzle.slot[1]>=0)){ showMsg('先に置き場Bを置いてください', true); return; }
    puzzle.grid[i]=0; putMarkGoal(2,i);
  }else if(tool==='slot1'){ puzzle.grid[i]=0; putMarkGoal(0,i); }
  else if(tool==='slot2'){
    if(!(puzzle.slot&&puzzle.slot[0]>=0)){ showMsg('先に置き場Aを置いてください', true); return; }
    puzzle.grid[i]=0; putMarkGoal(1,i);
  }else if(tool==='water'){
    vacate(i);
    puzzle.grid[i]=0;
    if(!puzzle.water) puzzle.water=new Uint8Array(puzzle.grid.length);
    puzzle.water[i]=1;
  }else if(tool==='man'){
    vacate(i);
    puzzle.grid[i]=0;
    player=i;
    if(RULE==='duo'){ const k=bees.indexOf(i); if(k>=0) bees.splice(k,1); }
    if(RULE==='ants'){ const k=coants.findIndex(a=>a.at===i); if(k>=0) coants.splice(k,1); }
  }else if(tool==='ant'){                      // 蟻。同僚を置く / もう一度押すと消す
    vacate(i);
    puzzle.grid[i]=0;
    const k=coants.findIndex(a=>a.at===i);
    if(k>=0) coants.splice(k,1);
    else { coants.push({at:i, claim:-1, dir:-1}); coants.sort((x,y)=>x.at-y.at); if(i===player) player=-1; }
  }else if(tool==='mate'){                     // 春。2匹目を置く
    vacate(i);
    puzzle.grid[i]=0;
    if(i===player) player=-1;
    // 押すたびに増やす/消す。並び順は盤の左上から
    const k=bees.indexOf(i);
    if(k>=0){ bees.splice(k,1); if(turn>=bees.length) turn=0; }
    else { bees.push(i); bees.sort((x,y)=>x-y); }
    if(bees.length && bees.indexOf(player)<0){ turn=0; player=bees[0]; }
    else turn=Math.max(0, bees.indexOf(player));
  }
  applyPieces();
  editDirty=true;
  dist=null;
...
function editProblem(){
  ...
  if(RULE==='marks'){
    const nm=(puzzle.num||[]).length, ns=(puzzle.slot||[]).length;
    if(nm!==ns) return '印つきの荷物 '+nm+' に対して印つきの置き場 '+ns;
    const nfree=boxes.length-nm;
    const nopen=[...goalSet].filter(g=>(puzzle.slot||[]).indexOf(g)<0).length;
    if(nfree!==nopen) return '印の無い荷物 '+nfree+' に対して印の無い置き場 '+nopen;
  }
  if(RULE==='water' && puzzle.water){
    for(const b of boxes) if(puzzle.water[b]) return SKIN.box+' は水の上に置けません';
  }
  if(RULE==='duo' && bees.length<2) return 'ミツバチが1匹しかいません（もう1匹置いてください）';
  if(RULE==='ants' && !coants.length) return '同僚のアリがいません';
...
function editCheck(){
  ...
  showMsg('確かめています…');
  const b=boardToXSB();
  const wr=window.WarehouseRules && WarehouseRules.RULES[RULE];
  // ルール違いはふつうの詰み表では手数も可否も違う。rules.js でそのルールのまま解く。
  // 無いのにふつうの表へ落とすと、水の上へ荷物を押し込めることになって狂う
  if(RULE!=='plain'){
    if(!wr){ showMsg('このルールではまだ確かめられません', true); return null; }
    return new Promise(res=>setTimeout(()=>{
      try{
        const p=wr.parse(b);
        let layer=[wr.start(p)];
        const seen=new Set([wr.key(layer[0])]);
        for(let d=0; d<=200; d++){
          if(layer.some(s=>wr.solved(p,s))){
            showMsg('解けます。最短'+d+'手');
            return res({pushes:d});
          }
          const next=[];
          for(const st of layer) for(const m of wr.moves(p,st)){
            const k=wr.key(m.st);
            if(seen.has(k)) continue;
            seen.add(k); next.push(m.st);
          }
          if(!next.length){
            showMsg('この配置からは解けません', true);
            return res(null);
          }
          layer=next;
          layer=next;
        }
        showMsg('手数が多すぎて確かめられません', true); res(null);
      }catch(e){ showMsg(String(e.message||e), true); res(null); }
    },0));
  }
  return new Promise(res=>setTimeout(()=>{
...
async function editExport(){
  ...
  if(a.trapRatio!==undefined){
    item.tr=Math.round(a.trapRatio*100);
    item.f=a.forced; item.g=a.greedyDied; item.og=a.offGoal?1:0;
  }
...
/* ================= 止まったときの記録 =================
   「動かせなくなり、何の操作も受け付けなくなる」を調べるための控え。
   入力を止めうるものを全部その場に出し、同じものを localStorage にも書く。
   **画面ごと固まって読めなくても、読み直したあとに「前回の記録」として見られる。**

   何を見るか:
   ・止まり … 画面が1コマも描けなかった時間。0.7秒以上をここに残す。
              **これが出ていれば、ゲームの掛け金ではなく画面ごと固まっている**
              (音・媒体まわりや端末側の詰まり)
   ・触     … 指やキーが届いた回数。素の event を数えているので、
              ここが増えていなければ、そもそも入力が画面に届いていない
   ・弾     … move() が何で弾かれたか。触は届いているのに動かないなら、ここに出る
   ・掛け金 … finished / EDIT / antBusy / coop / tracking
   ・覆い   … かぶさっている板。ここが「なし」でないと、タップは全部そこで止まる  */
const DIAG_KEY='warehouse-diag';
let diagPrev=null;
try{ const t=localStorage.getItem(DIAG_KEY); if(t) diagPrev=JSON.parse(t); }catch(e){}
const diag={ 鼓動:0, 最後の描画:Date.now(), 止まり:[], 盤:[], 画面:0,
             触:{押下:0,触始:0,触動:0,触終:0,click:0,鍵:0},
             最後に触れた:0, 弾:'', 弾いた回数:0, 最後に動いた:0 };
function diagBlock(why){
  if(!why){ diag.最後に動いた=Date.now(); diag.弾=''; return; }
  diag.弾=why; diag.弾いた回数++;
}
function diagSnap(){
  const ago=t=>t?Math.round((Date.now()-t)/100)/10+'秒前':'なし';
  let ov='なし';
  try{
    const a=[...document.querySelectorAll('.overlay.show,.favpanel.show')]
      .map(e=>e.id||e.className);
    if(a.length) ov=a.join(' ');
  }catch(e){}
  return {
    時刻:new Date().toTimeString().slice(0,8),
    入口:(window.GAME&&GAME.pack)||location.pathname, 規則:RULE,
    面:puzzle?(index+1):null,
    鼓動:diag.鼓動, 描画:ago(diag.最後の描画),
    止まり:diag.止まり.slice(-6),
    盤の大きさ:getComputedStyle(document.documentElement).getPropertyValue('--cell').trim(),
    盤が変わった:diag.盤.slice(-6),
    画面が変わった回数:diag.画面,
    動きを止めている:!ANIM,
    触:Object.assign({},diag.触), 触った:ago(diag.最後に触れた),
    動いた:ago(diag.最後に動いた), 弾:diag.弾||'なし', 弾いた回数:diag.弾いた回数,
    掛け金:{finished:!!finished, EDIT:!!EDIT,
            antBusy:(typeof antBusy!=='undefined')&&!!antBusy,
            coop:!!coop, tracking:!!tracking},
    覆い:ov, 隠れ:!!document.hidden,
    表:{作り方:tableHow, 大きさ:dist?dist.size:null, 作れず:tableOff,
        かかった:tableMs+'ms', Workerの失敗:tableFails},
    音を切っている:!SOUND,
    音:{状態:!AC?'非対応':!actx?'未作成':actx.state,
        断り媒:mediaNo, 断り波:wavNo, 波の数:Object.keys(WAV).length,
        触:audioLog.touch, 作:audioLog.make, 起:audioLog.resume, 波:audioLog.wav,
        道:audioLog.ses||'-', 誤:audioLog.err||'-'},
  };
}
function saveDiag(){ try{ localStorage.setItem(DIAG_KEY, JSON.stringify(diagSnap())); }catch(e){} }
(function diagStart(){
  const map={pointerdown:'押下',touchstart:'触始',touchmove:'触動',touchend:'触終',click:'click',keydown:'鍵'};
  for(const ev of Object.keys(map))
    document.addEventListener(ev, ()=>{ diag.触[map[ev]]++; diag.最後に触れた=Date.now(); },
                              {passive:true, capture:true});
  let last=performance.now();
  const tick=now=>{
    const gap=now-last; last=now;
    diag.鼓動++; diag.最後の描画=Date.now();
    // 0.7秒ひとコマも描けていなければ、そのあいだ画面ごと止まっていたということ
    if(gap>700){
      diag.止まり.push(new Date().toTimeString().slice(0,8)+' '+(Math.round(gap)/1000).toFixed(1)+'秒');
      if(diag.止まり.length>12) diag.止まり.shift();
    }
    requestAnimationFrame(tick);
  };
  requestAnimationFrame(tick);
  // iPhone はアドレスバーの出入りで画面の高さが変わる。そのたびに盤を測り直すので、
  // 遊んでいる最中に起きていないかを数える
  const onResize=()=>{ diag.画面++; };
  window.addEventListener('resize', onResize);
  window.addEventListener('orientationchange', onResize);
  if(window.visualViewport) window.visualViewport.addEventListener('resize', onResize);
  setInterval(saveDiag, 1000);
  window.addEventListener('pagehide', saveDiag);
})();
let diagTimer=null;
function showDiag(){
  const el=$('diagNote');
  if(el.style.display!=='none'){ el.style.display='none'; clearInterval(diagTimer); diagTimer=null; return; }
  el.style.display='block';
  const draw=()=>{
    el.textContent =
      (diagPrev ? '【前回の記録（読み直す前）】\n'+JSON.stringify(diagPrev,null,1)+'\n\n' : '')
      + '【いま】\n'+JSON.stringify(diagSnap(),null,1);
  };
  draw();
  diagTimer=setInterval(draw, 500);
}
...
/* iPhone は画面のいちばん下の帯(ホームバー)からのスワイプが「アプリの切り替え」。
   その帯で始まった指をこちらで受けると、切り替えようとしただけで
   「上へ動かす」が入ってしまう(本人の報告)。
   まず CSS で安全域ぶんを空けて、その帯にボタンを置かないようにしてある。
   そのうえで、帯そのもので始まった指は受けない。
   受けないので preventDefault もせず、iOS の操作の邪魔もしない。
   ホームバーの無い端末(安全域0)では、この帯も0になり、今までどおり */
function bottomEdge(){
...
document.addEventListener('touchstart',e=>{
  ...
  const edge=bottomEdge();
  const h=(window.visualViewport&&window.visualViewport.height)||window.innerHeight;
  if(edge>0 && t0 && h-t0.clientY < edge){ tracking=false; return; }   // ホームバーの帯
...
$('btnAnimOff').addEventListener('click',()=>{
  try{ localStorage.setItem('warehouse-anim', ANIM?'0':'1'); }catch(e){}
  const u=new URL(location.href);
  u.searchParams.delete('anim');             // 覚えた値が効くよう、指定は消して読み直す
  location.href=u.pathname+(u.search||'');
});
$('btnSoundOff').addEventListener('click',()=>{
  try{ localStorage.setItem('warehouse-sound', SOUND?'0':'1'); }catch(e){}
  const u=new URL(location.href);
  u.searchParams.delete('sound');            // 覚えた値が効くよう、指定は消して読み直す
  location.href=u.pathname+(u.search||'');
});
$('btnSoundTest').addEventListener('click',()=>{
  ...
  try{
    if(actx && actx.state==='running'){
      ...
      o.connect(g).connect(getMaster()||actx.destination); o.start(t); o.stop(t+0.6);
...
/* ルールの選択。デバッグ表示のときにしか押せない。
   前は「本編 ⇔ 夏」を行き来するだけのトグルで、他のルールへは行けなかった。
   全部のルールを一覧にして選べるようにした。
   どれも別の入口(頁)なので、開き直すだけで状態は持ち回らない */
$('btnPack').textContent='ルールを選ぶ';
$('btnPack').addEventListener('click',()=>{
  const list=$('modeList');
  list.innerHTML='';
  for(const m of MODES){
    const here = m.pack===PACK;
    const el=document.createElement(here?'span':'a');   // いまいる所は押せない
    el.className='mode'+(here?' now':'');
    if(!here) el.href='../'+m.dir+'/';
    const em=document.createElement('span'); em.className='em'; em.textContent=m.emoji;
    const info=document.createElement('span'); info.className='info';
    const nm=document.createElement('span'); nm.className='nm';
    nm.textContent=m.name+(here?'（いまここ）':'');
    const ds=document.createElement('span'); ds.className='ds'; ds.textContent=m.desc;
    info.appendChild(nm); info.appendChild(ds);
    el.appendChild(em); el.appendChild(info);
    list.appendChild(el);
  }
  $('modeOverlay').classList.add('show');
});
$('btnModeClose').addEventListener('click',()=>$('modeOverlay').classList.remove('show'));
...
$('btnEditEnter').addEventListener('click',()=>{
  const u=new URL(location.href);
  u.searchParams.set('edit','1');
  u.searchParams.set('lv', String(index+1));
  location.href=u.pathname+u.search;
});
$('btnEditExit').addEventListener('click',()=>{
  const u=new URL(location.href);
...
/* ルール違いのゲームは、まず「ふつうと何が違うか」を先頭に出す。
   これが無いと、ルールの説明が本編のままになってしまう */
const RULE_TEXT={
  water:'水のマスがあります。<b>'+SKIN.man+' は水の上を歩けます</b>が、'
       +'<b>'+SKIN.box+' は水に押し込めません</b>。',
  holes:'置き場は<b>穴</b>です。'+SKIN.box+' を落として埋めるまで、<b>穴は通れません</b>。'
       +'埋めた穴は地面と同じになり、その上を歩けます。埋めた '+SKIN.box+' は<b>もう動かせません</b>。',
  slide:SKIN.box+' を押すと、<b>壁か別の '+SKIN.box+' にぶつかるまで滑って</b>いきます。'
       +'途中では止まりません。押した '+SKIN.man+' は<b>その場に留まります</b>。',
  marks:SKIN.box+' と置き場に <b>A B C…</b> の印が付いていることがあります。'
       +'<b>A は A の置き場へ、B は B の置き場へ</b>入れてください。'
       +'<b>印の無い '+SKIN.box+'</b> は、<b>印の無い置き場</b>へ。'
       +'違うところに乗せると<b>赤</b>になります。',
  roll:SKIN.box+' は押すたびに<b>時計回りに90度</b>まわります。置き場に乗せるだけでなく、'
       +'<b>向きが元に戻っている</b>必要があります(=それぞれを押した回数が4の倍数)。',
  ants:'ステージには<b>同僚のアリ</b>がいます（色が薄いほう）。濃いほうがあなたです。'
       +'あなたが '+SKIN.box+' を1つ押すと、<b>同僚もそれぞれ1つ押します</b>。'
       +'同僚は<b>自分から一番近い '+SKIN.box+'</b> へ、<b>曲がる回数がいちばん少ない道</b>で近づき、'
       +'<b>押す場所に立ったら、その場で向きを変えて</b>押します。'
       +'一度押しはじめた '+SKIN.box+' には<b>そのまま付いて回り</b>、'
       +'その向きに押せなくなったら回り込んで別の向きから押します。'
       +'<b>置き場に収まった '+SKIN.box+'</b> と、<b>あなたが直前に押した '+SKIN.box+'</b> には触りません。'
       +'押すものが無い同僚は、その場にいます。'
       +'同僚のマスには入れず、そこへ押し込むこともできません。',
  duo:SKIN.man+' は<b>何匹か</b>います。動かせるのは<b>輪のついた1匹</b>だけで、'
       +'<b>1回押すごとに、盤の左上からの順ぐりで交代</b>します'
       +'(押さずに歩くだけでは交代しません)。'
       +'待っているほうは<b>壁</b>になり、通り抜けも、そこへ押し込むこともできません。',
};
if(RULE_TEXT[RULE]){
  const note=document.createElement('p');
  note.className='rulenote';
  note.innerHTML=RULE_TEXT[RULE];
  const rules=document.querySelector('.rules');
  rules.insertBefore(note, rules.firstChild);
}
