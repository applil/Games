...
const PACKS={
  ...
  number:  {file:'packs/number.json',   skin:'normal',   label:'印あわせ',   rule:'numbered', man:'🐣'},
...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    if(RULE==='numbered'){
      ...
      // 印が合っていれば緑。置き場に乗っていても合っていないなら赤にして、
      // 「置き場ではあるが、この荷物ではない」と分かるようにする。
      // 印のない荷物は、印のない置き場ならどこでもよい
      if(bn>=0){
        const fit = markedBox ? boxes[bn]===puzzle.slot[bn] : (goal && gn<0);
        el.classList.toggle('on', fit);
        el.classList.toggle('miss', goal && !fit);
      }
    }
  }
}
...
function checkWin(){
  ...
  // 印あわせは、印の合う置き場に乗っていないとだめ。
  // 印のない荷物は、印のない置き場ならどこでもよい
  else if(RULE==='numbered'){
    const nm=puzzle.nmark|0, marked=new Set(puzzle.slot);
    if(!boxes.every((b,k)=> k<nm ? b===puzzle.slot[k] : (goalSet.has(b) && !marked.has(b)))) return false;
  }
...
const RULE_TEXT={
  ...
  numbered:SKIN.box+' と置き場に <b>A B C…</b> の印が付いていることがあります。'
       +'印の付いた '+SKIN.box+' は、<b>同じ印の置き場</b>へ。'
       +'印の無い '+SKIN.box+' は、<b>印の無い置き場ならどこでも</b>かまいません。'
       +'合っていない置き場に乗せると<b>赤</b>になります。'
