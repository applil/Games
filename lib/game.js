...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    if(RULE==='ants'){
      el.classList.toggle('you', i===player && !box);
    ...
    if(RULE==='marks'){
      ...
      // 印の無いほうは、荷物も置き場もどれでも受ける
      if(bn>=0){
        const clash = markedBox && gn>=0 && gn!==bn;
        el.classList.toggle('on', goal && !clash);
        el.classList.toggle('miss', goal && clash);
      }
    }
  }
  // 蟻。同僚が今どの荷物を狙っているかを出しておく。
  // 押したあとの配置では変わることもあるが、「一番近い荷物を、その向きで押す」が
  // 見えていないと、同僚が勝手に動いたようにしか見えない
  if(RULE==='ants' && !antBusy){
    const w=puzzle.w;
    coants.forEach((_, who)=>{
      const aim=pickAntPush(who, boxes, coants, player);
      if(!aim) return;
      const antEl=cellEls[coants[who]];
      if(antEl && antEl.firstChild){
        const t=document.createElement('span');
        t.className='antaim';
        t.textContent=aim.dir===-w?'↑':aim.dir===w?'↓':aim.dir===-1?'←':'→';
        antEl.appendChild(t);
      }
      if(cellEls[aim.box]) cellEls[aim.box].classList.add('aim');
    });
  }
}
  ...
  if(bi>=0){
    ...
    if(delayAnts){
      ...
      setTimeout(()=>{
        ...
        antBusy=false;
        paint();
        for(const a of later){ slide(a.to, a.dir); slide(a.at, a.dir); }
        updateStats();
        if(checkWin()) return;
        if(DEBUG) checkDead();
      }, SLIDE_MS+40);
...
function checkWin(){
  ...
  // 印あわせ。全部が置き場に乗っていて、かつ「印つきの荷物が、ちがう印の置き場に
  // 乗っている」ことが1つも無ければクリア。印の無い荷物・置き場は万能札
  else if(RULE==='marks'){
    const nm=puzzle.nmark|0;
    for(let k=0;k<boxes.length;k++){
      const b=boxes[k];
      if(!goalSet.has(b)) return false;
      if(k<nm){ const gn=puzzle.slot.indexOf(b); if(gn>=0 && gn!==k) return false; }
    }
  }
...
function boardToXSB(){
  ...
  for(let y=0;y<puzzle.h;y++){
    ...
    for(let x=0;x<puzzle.w;x++){
      ...
      if(puzzle.water && puzzle.water[i] && i!==player && !boxSet.has(i) && !goalSet.has(i)){
        r+='~'; continue;
      }
...
const RULE_TEXT={
  ...
  marks:SKIN.box+' と置き場に <b>A B C…</b> の印が付いていることがあります。'
       +'だめなのは<b>印の付いた '+SKIN.box+' を、ちがう印の置き場に置く</b>ことだけです。'
       +'<b>印の無い置き場</b>は、印の付いた '+SKIN.box+' でも受けます。'
       +'<b>印の無い '+SKIN.box+'</b> は、印の付いた置き場にも入れます。'
       +'置けないところに乗せると<b>赤</b>になります。'
       +'面によって、全部に印が付いていたり、一部だけだったりします。',
  roll:SKIN.box+' は押すたびに<b>時計回りに90度</b>まわります。置き場に乗せるだけでなく、'
       +'<b>向きが元に戻っている</b>必要があります(=それぞれを押した回数が4の倍数)。',
  ants:'ステージには<b>同僚のアリ</b>がいます（色が薄く、矢印が付いているほう）。'
       +'輪のついたアリがあなたです。あなたが '+SKIN.box+' を1つ押すと、'
       +'<b>少し遅れて同僚もそれぞれ1つずつ押して</b>しまいます。同僚は'
       +'<b>自分から一番近い '+SKIN.box+'</b> を、<b>矢印の向きのまま</b>押します。'
       +'押せないときは動きません。同僚のいるマスには入れず、そこへ押し込むこともできません。',
