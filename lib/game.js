...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    if(RULE==='ants'){
      ...
      // 押す向きは局面の一部なので、盤に出しておく。出さないと先が読めない
      if(ant && ant.dir>=0){
        const t=document.createElement('span');
        t.className='antaim';
        t.textContent=['↑','↓','←','→'][ant.dir];
        el.appendChild(t);
      }
    }
    if(RULE==='duo'){
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    if(acts) for(const a of acts){
      if(a.push){ slide(a.to, a.dir); slide(a.at, a.dir); }
      else slide(a.at, a.at-a.from);
    }
...
function antStep(who, boxArr, antArr, mePlayer, taboo){
  ...
  if(me.at===stand){
    ...
    me.at=b; me.claim=to;
    return {push:true, from:stand, at:b, box:b, dir, to};
  }
  const back=walk(stand);
  ...
  const from=me.at;
  me.at=go;
  return {push:false, from, at:go, dir:D[gk]};
}
...
async function playAnts(seq, done){
  ...
  for(let who=0; who<coants.length; who++){
    ...
    if(a.push){
      ...
      slide(a.at, a.at-a.from, ANT_STEP_MS);
