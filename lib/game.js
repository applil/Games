...
let coants=[];             // 蟻: 同僚の居場所。こちらが押すたびに1つずつ押す
...
function startLevel(i){
  ...
  coants = puzzle.ants ? puzzle.ants.slice() : [];                     // 蟻。同僚
  dropDone();
...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    const coworker = RULE==='ants' && coants.indexOf(i)>=0;
...
function move(name, fromReplay){
  ...
  // 蟻。同僚のいるマスには入れない
  if(RULE==='ants' && coants.indexOf(next)>=0){ SFX.blocked(); return; }
  ...
  if(bi>=0){
    ...
    // 蟻。同僚には押し込めない
    if(RULE==='ants' && coants.indexOf(beyond)>=0){ SFX.blocked(); return; }
    ...
    undoStack.push({player, mate, ants:coants.slice(), boxes:boxes.slice(), pushCount, moveCount, filled:new Set(filled), rot:rot.slice()});
    ...
    const acts = (RULE==='ants' && !delayAnts) ? runAnts() : null;
    pushCount++; moveCount++;
    paint();
    if(skinName==='winter'){
    ...
    // 蟻。同僚が押したぶんも滑らせる。歩いてきた道のりは出さず、最後の1マスだけ
    ...
    undoStack.push({player, mate, ants:coants.slice(), boxes:boxes.slice(), pushCount, moveCount});
    player=next;
    moveCount++;
...
/* 蟻。こちらが1つ押すと、同僚も1匹ずつ押す。
   ...
   返すのは、動いた蟻の記録。演出に使う */
function pickAntPush(who, boxArr, antArr, mePlayer){
  const {grid,w}=puzzle;
  const me=antArr[who];
  const blocked=new Set(boxArr);
  antArr.forEach((a,i)=>{ if(i!==who) blocked.add(a); });
  ...
  for(const b of boxArr.slice().sort((x,y)=>x-y)){
    for(let k=0;k<4;k++){
      ...
      if(!best || d<best.d || (d===best.d && (k<best.k || (k===best.k && b<best.box))))
        best={d,k,box:b,dir,to,stand};
    }
  }
  if(!best) return null;
  // 押す位置までの道。最後の1マスだけ飛ばすと、同僚がワープしたように見える
  const path=[];
  let c=best.stand;
  path.push(c);
  while(c!==me){
...
function runAnts(){
  ...
  for(let who=0; who<coants.length; who++){
    const best=pickAntPush(who, boxes, coants, player);
    if(!best) continue;                      // 押せないので動かない
    boxes[boxes.indexOf(best.box)]=best.to;
    coants[who]=best.box;                    // ダンボールがいた場所へ入る
    acts.push({at:best.box, dir:best.dir, to:best.to});
  }
  return acts;
}
...
async function playAnts(seq, done){
  ...
  for(let who=0; who<coants.length; who++){
    ...
    const best=pickAntPush(who, boxes, coants, player);
    if(!best) continue;
    const word=antDirWord(best.dir);
    showMsg('同僚が、一番近い '+SKIN.box+' を'+word+'へ押します');
    if(cellEls[best.box]) cellEls[best.box].classList.add('antbox');
    const path=best.path||[coants[who], best.stand];
    ...
    coants[who]=best.box;
    paint();
    if(cellEls[best.to]) cellEls[best.to].classList.add('antbox');
    slide(best.to, best.dir, ANT_PUSH_MS);
    slide(best.box, best.dir, ANT_PUSH_MS);
    if(!await antWait(ANT_PUSH_MS+80, seq)) return;
  }
  hideMsg();
  if(puzzle.meta && puzzle.meta.note){
    $('banner').style.display='block';
    $('bannerText').textContent='💡 '+puzzle.meta.note;
  }
  done();
}
function bump(i){
...
function resetLevel(){
  ...
  coants = puzzle.ants ? puzzle.ants.slice() : [];
  dropDone();
  undoStack=[]; pushCount=0; moveCount=0;
  deadShown=false;
  paint(); updateStats(); clearHint(); hideMsg();
...
function boardToXSB(){
  ...
  for(let y=0;y<puzzle.h;y++){
    ...
    for(let x=0;x<puzzle.w;x++){
      ...
      if(RULE==='ants' && coants.indexOf(i)>=0){ r+='&'; continue; }
...
function editCell(i){
  ...
  if(tool==='wall'){
    ...
    if(RULE==='ants'){ const k=coants.indexOf(i); if(k>=0) coants.splice(k,1); }
  }else if(tool==='ant'){                      // 蟻。同僚を置く / もう一度押すと消す
    vacate(i);
    puzzle.grid[i]=0;
    const k=coants.indexOf(i);
    if(k>=0) coants.splice(k,1);
    else { coants.push(i); coants.sort((x,y)=>x-y); if(i===player) player=-1; }
