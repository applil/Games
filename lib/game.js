...
function parseBoard(board){
  ...
  return num.length
    ? {grid, water, w, h, boxes:num.slice(), goals:slot.slice(), num:num.slice(), slot:slot.slice(), player}
    : {grid, water, w, h, boxes:boxes.sort((a,b)=>a-b), goals:goals.sort((a,b)=>a-b), player};
}
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    undoStack.push({player, boxes:boxes.slice(), pushCount, moveCount, filled:new Set(filled), rot:rot.slice()});
