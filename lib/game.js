...
function antStep(who, boxArr, antArr, mePlayer, taboo){
  const {grid,w}=puzzle;
  const D=[-w,w,-1,1];
  const me=antArr[who];
...
async function playAnts(seq, done){
  if(!await antWait(SLIDE_MS+80, seq)) return;
  const taboo=antTaboo;
  for(let who=0; who<coants.length; who++){
    if(seq!==antSeq || finished) return;
    const before=coants[who].at;
    const a=antStep(who, boxes, coants, player, taboo);
    if(!a) continue;
    showMsg('同僚が '+SKIN.box+' を'+antDirWord(a.dir)+'へ押します');
    const path=a.path||[before, a.from];
    const at=coants[who].at;
    coants[who].at=before;                    // いったん元の場所へ戻して、歩いて見せる
    for(let i=1;i<path.length;i++){
      if(seq!==antSeq || finished){ coants[who].at=at; return; }
      coants[who].at=path[i];
      paint();
      if(cellEls[a.box]) cellEls[a.box].classList.add('antbox');
      slide(path[i], path[i]-path[i-1], ANT_STEP_MS);
      if(!await antWait(ANT_STEP_MS+40, seq)){ coants[who].at=at; return; }
    }
    coants[who].at=at;
    paint();
    if(cellEls[a.to]) cellEls[a.to].classList.add('antbox');
    slide(a.to, a.dir, ANT_PUSH_MS);
    slide(a.at, a.dir, ANT_PUSH_MS);
