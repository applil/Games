...
/* 蟻を1匹だけ進める。規則は warehouse/rules.js の ants と同じにしてある
   (片方だけ直さないこと。画面と探索で答えが食い違う)。
     ...
     ・取り付いたら、その荷物に執着する
     ・いまの向きに押せなければ、左右の近いほう、だめなら反対側へ回り込む
     ・1手に1マスだけ歩く。押す位置に着いた次の手で押す
...
function antStep(who, boxArr, antArr, mePlayer, taboo){
  ...
  if(me.claim<0){
    ...
    for(const b of boxArr.slice().sort((x,y)=>x-y)){
      ...
      for(let k=0;k<4;k++){
        ...
        if(grid[to]||blocked.has(to)) continue;
  ...
  if(ok(me.dir)<0){
    ...
    if(!pick){ const d=ok(me.dir^1); if(d>=0) pick={k:me.dir^1,d}; }
  ...
  const path=[];
  {
    ...
    while(c!==me.at){
      const d=dist.get(c);
      let nxt=-1;
      for(const k of [0,1,2,3]){ const n=c+D[k]; if(dist.get(n)===d-1){ nxt=n; break; } }
      if(nxt<0) break;
      path.push(nxt); c=nxt;
    }
    path.reverse();
