...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    // 印はどちらも絵の中(.fig)に入れる。大きさも位置も同じになり、
    // 荷物の印は押したとき絵と一緒に動く。
    // 同じ場所なので、荷物が乗っているマスでは荷物の印が出る
    // (置き場の印を見たければ、荷物をどければよい)
    if(RULE==='numbered'){
      const bn=boxes.indexOf(i), gn=puzzle.slot.indexOf(i);
      const mark = bn>=0 ? MARKS[bn] : (gn>=0 ? MARKS[gn] : '');
      if(mark){
        const t=document.createElement('span');
        t.className='tag'+(bn<0?' goaltag':'');   // 置き場の印は色だけ変える
        t.textContent=mark;
        fig.appendChild(t);
      }
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    if(drop){ boxes.splice(bi,1); rot.splice(bi,1); filled.add(dest); }   // 穴に落ちて埋まる
    else boxes[bi]=dest;
    ...
    if(goalSet.has(dest)){ SFX.onGoal(); setTimeout(()=>bump(dest), RULE==='roll'?ROLL_MS+SLIDE_MS:110); }
    else SFX.push();
  }else{
...
/* フンコロガシ。押すと、まずその場で90度まわり、それから隣のマスへ転がって移る。
   移動には transform ではなく left/top を使う。
   CSS は rotate プロパティを transform より先に掛けるので、
   同じ要素で両方を使うと、傾いたぶんだけ移動の向きまで回ってしまう
   (右へ押したのに下から入ってくる、といったことが起きる)。
   left/top なら向きと位置が混ざらない */
const ROLL_MS=180;                        // 回る時間。.cell .fig の rotate と合わせる
function roll(i, d, from, to){
  ...
  void fig.offsetWidth;
  fig.classList.remove('sliding');
  fig.style.rotate=to+'deg';              // その場で回る
  setTimeout(()=>{                        // 回り終わってから、転がって移る
    // 空文字に戻すと left が auto になり、auto とは繋がらないので飛んでしまう。
    // 0px を書いて動かす。別の手が始まっていたら触らない
    if(fig.style.left===dx+'px' && fig.style.top===dy+'px'){
      fig.style.left='0px'; fig.style.top='0px';
    }
  }, ROLL_MS);
}
