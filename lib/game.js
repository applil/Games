...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    else if(goalSet.has(dest)){ SFX.onGoal(); setTimeout(()=>bump(dest),110); }
    else SFX.push();
    if(delayAnts){
      antBusy=true;
      const seq=++antSeq;
...
/* 何マスも滑るときに、同じ時間で行かせると速すぎる。
   ...
   間をとって、距離の0.65乗で伸ばす。1マスは今までどおり0.5秒、
   4マスで1.23秒(速さは2→3.3マス毎秒)。長すぎても待たされるので上限を置く */
...
function editCheck(){
  ...
  if(RULE!=='plain'){
    ...
    return new Promise(res=>setTimeout(()=>{
      try{
        const p=wr.parse(b);
        // いま塗ってある水を優先する。文字に落とす途中で足元の水が落ちないように
        if(RULE==='water' && puzzle.water) p.water=puzzle.water.slice();
        ...
        for(let d=0; d<=200; d++){
          ...
          if(!next.length){ showMsg('この配置からは解けません', true); return res(null); }
        }
