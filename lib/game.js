...
const SFX_SETS={
  ...
  winter:{
    ...
    push(ms){ noise(Math.max(0.5,(ms||500)/1000), 0,
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    if(skinName==='winter'){
      ...
      setTimeout(()=>{ SFX.onGoal(); bump(dest); }, SLIDE_MS);
    }
    else SFX.push();
    if(delayAnts){
      antBusy=true;
...
const GLIDE_MS=500;                       // 冬の氷。1マスぶん滑るときの時間
/* 何マスも滑るときに、同じ時間で行かせると速すぎる。
   かといって距離ぶんだけ時間を伸ばすと、遠いときに間延びする。
   間をとって、距離の0.65乗で伸ばす。1マスは0.5秒、
   4マスで1.23秒。長すぎても待たされるので上限を置く */
const glideMs=n=>Math.min(1300, Math.round(GLIDE_MS*Math.pow(Math.max(1,n||1), 0.65)));
