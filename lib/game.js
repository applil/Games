...
const PACKS={
  ...
  number:  {file:'packs/number.json',   skin:'normal',   label:'番号つき',   rule:'numbered', man:'🐣'},
...
function parseBoard(board){
  ...
  const boxes=[], goals=[], num=[], slot=[], players=[];
  let player=-1;
  for(let y=0;y<h;y++){
    ...
    for(let x=0;x<w;x++){
      ...
      // 番号つき。荷物は 1〜9、置き場は a〜i。番号の合う置き場でないとクリアにならない
      const ni=NUMS.indexOf(c), gi=GOALS.indexOf(c);
      if(ni>=0){ num[ni]=i; boxes.push(i); continue; }
      if(gi>=0){ slot[gi]=i; goals.push(i); continue; }
      if(c==='$'||c==='*') boxes.push(i);
  ...
  // 番号つきは、荷物の並び順が「どの番号か」を表すので並べ替えない
  return num.length
    ? {grid, water, w, h, boxes:num.slice(), goals:slot.slice(), num:num.slice(), slot:slot.slice(), players, player}
    : {grid, water, w, h, boxes:boxes.sort((a,b)=>a-b), goals:goals.sort((a,b)=>a-b), players, player};
}
...
function buildBoard(){
  ...
  for(let i=0;i<puzzle.grid.length;i++){
    ...
    fig.className='fig';
    d.appendChild(fig);
    // 番号あわせの、置き場の印を置く台。絵とまったく同じ大きさ・同じ場所で、
...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    if(RULE==='numbered'){
      ...
      // 印が合っていれば緑。置き場に乗っていても印が違うなら赤にして、
      // 「置き場ではあるが、この荷物ではない」と分かるようにする
      if(bn>=0){
        const fit = boxes[bn]===puzzle.slot[bn];
        el.classList.toggle('on', fit);
        el.classList.toggle('miss', gn>=0 && !fit);
      }
    }
  }
}
...
function checkWin(){
  ...
  // 番号つきは、番号の合う置き場に乗っていないとだめ
  else if(RULE==='numbered'){ if(!boxes.every((b,k)=>b===puzzle.slot[k])) return false; }
...
const RULE_TEXT={
  ...
  numbered:SKIN.box+' と置き場に <b>A B C…</b> の印があります。<b>印の合う置き場</b>に置かないと'
       +'クリアになりません。印の違う置き場に乗せると<b>赤</b>になります。',
