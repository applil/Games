...
document.body.insertAdjacentHTML("afterbegin", `<div class="app">
  ...
  <div class="palette" id="palette">
    ...
    <button data-tool="mate" class="pal-duo">2匹目</button>
...
let mate=-1;               // 春: いま番でないミツバチの居場所(壁になる)
...
function startLevel(i){
  ...
  mate = RULE==='duo' && puzzle.players ? (puzzle.players[1]|0) : -1;   // 春。2匹目
...
function move(name, fromReplay){
  ...
  if(RULE==='duo' && next===mate){ SFX.blocked(); return; }
  // 蟻。同僚のいるマスには入れない
  ...
  if(bi>=0){
    ...
    // 春。相方には押し込めない
    if(RULE==='duo' && beyond===mate){ SFX.blocked(); return; }
    // 蟻。同僚には押し込めない
    ...
    undoStack.push({player, mate, ants:coants.map(a=>({at:a.at,claim:a.claim,dir:a.dir})), taboo:antTaboo, boxes:boxes.slice(), pushCount, moveCount, filled:new Set(filled), rot:rot.slice()});
    ...
    if(RULE==='duo'){ player=mate; mate=pusher; }
    ...
    undoStack.push({player, mate, ants:coants.map(a=>({at:a.at,claim:a.claim,dir:a.dir})), taboo:antTaboo, boxes:boxes.slice(), pushCount, moveCount});
    player=next;
    moveCount++;
...
function undo(){
  ...
  if(s.mate!==undefined) mate=s.mate;
...
function resetLevel(){
  ...
  rot=boxes.map(()=>0);
  mate = RULE==='duo' && puzzle.players ? (puzzle.players[1]|0) : -1;
...
function editCell(i){
  ...
  if(tool==='wall'){
    ...
    puzzle.grid[i]=0;
    player=i;
    if(RULE==='duo' && i===mate) mate=-1;      // 2匹目の上に置いたら、そちらは消す
...
function editProblem(){
  ...
  if(RULE==='duo' && mate<0) return 'ミツバチが1匹しかいません（2匹目を置いてください）';
