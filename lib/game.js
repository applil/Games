...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    // 荷物の札は絵の中に入れる。押したとき、絵と一緒に動いてほしいから。
    // 置き場の札は動かないので、マスのほうに出す
    if(RULE==='numbered'){
      ...
      const fit = bn>=0 && boxes[bn]===puzzle.slot[bn];
      el.dataset.num = gn>=0 ? MARKS[gn] : '';
      // 置き場の札は、空いているときと、印ちがいの荷物にふさがれているときに出す。
      // 合っている荷物の下では、同じ札が二重になるだけなので出さない
      el.classList.toggle('numgoal', gn>=0 && !fit);
