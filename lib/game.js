...
function paint(){
  ...
  for(let i=0;i<cellEls.length;i++){
    ...
    if(RULE==='ants') el.classList.toggle('mate', coworker);
    if(RULE==='duo'){
...
function move(name, fromReplay){
  ...
  if(bi>=0){
    ...
    // 蟻。こちらが押したので、同僚も1つずつ押す
    const acts = RULE==='ants' ? runAnts() : null;
    pushCount++; moveCount++;
    paint();
    if(skinName==='winter'){
...
function runAnts(){
  ...
  for(let who=0; who<coants.length; who++){
    ...
    coants.forEach((a,i)=>{ if(i!==who) blocked.add(a); });
    blocked.add(player);
    // 歩いた距離
    ...
    // 歩く距離が短い順、同じなら 上・下・左・右 の順、それも同じなら左上の荷物
    let best=null;
    for(const b of boxes.slice().sort((x,y)=>x-y)){
      for(let k=0;k<4;k++){
        const dir=[-w,w,-1,1][k];
        ...
        // 短い順 → 上下左右の順 → 盤の左上に近い荷物の順(rules.js と同じ)
...
const RULE_TEXT={
  ...
  ants:'ステージには<b>同僚のアリ</b>がいます。あなたが '+SKIN.box+' を1つ押すと、'
       +'<b>同僚もそれぞれ1つずつ押して</b>しまいます。同僚は'
       +'<b>自分から一番近い '+SKIN.box+'</b> を、<b>歩いてきた向きのまま</b>押します。'
